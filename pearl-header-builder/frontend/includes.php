				...
				foreach ( $columns as $column => $elements ) {
					foreach ( $elements as $data ) {
						...
						if ( ! empty( $element ) ) {
							...
							/*Generate text color*/
							if ( array_has( $data, 'data.textColor.name' ) && 'Custom' === $data['data']['textColor']['name'] ) {
								$custom_css .= $element . '{color: ' . $data['data']['textColor']['value'] . '}';
							}

							/*Generate icon color*/
							if ( array_has( $data, 'data.iconColor.name' ) && 'Custom' === $data['data']['iconColor']['name'] ) {
...
function stm_hb_element_style( $element ) {
	$style_class = '';
	if ( ! empty( $element['data'] ) && ! empty( $element['data']['style'] ) ) {
...
function stm_hb_get_dropdown( $dropdown ) {
	...
	if ( ! empty( $dropdown[0] ) && ! empty( $dropdown[0]['label'] ) ) {
...
function stm_hb_get_wpml_langs() {
	...
	if ( empty( $langs ) ) {
		$wpml = array(
			array(
				'label' => esc_html__( 'English', 'pearl-header-builder' ),
				'url'   => '/',
			),
		);
	}
